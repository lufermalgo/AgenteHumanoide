...
export const onAssessmentStart = functions.https.onCall(async (request) => {
...
function buildWavHeader(pcmBuffer: Buffer, sampleRate = 24000, channels = 1): Buffer {
...
export const tts = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const base64Audio: string | undefined = json?.candidates?.[0]?.content?.parts?.find((p: any) => p.inlineData)?.inlineData?.data;
    if (!base64Audio) {
...
export const stt = functions.https.onRequest(async (req, res) => {
  // CORS para desarrollo local
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type');
  ...
  try {
    ...
    const { audioBase64, mimeType = 'audio/wav' } = (req.body || {}) as { audioBase64?: string; mimeType?: string };
    if (!audioBase64) {
...
export const generate = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const { systemPrompt, userPrompt, maxTokens = 200 } = (req.body || {}) as { 
      systemPrompt?: string; 
      userPrompt?: string; 
      maxTokens?: number;
    };
